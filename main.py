...
from torch.utils.data import random_split, DataLoader, Dataset, BatchSampler
from torchmetrics.utilities import rank_zero_info

from ldm.data.base import Txt2ImgIterableBaseDataset
from ldm.data.reid import RandomIdentitySampler
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--test_only", type=str2bool, default=False, help="test only")
    parser.add_argument(
        "-e", "--extract_feature", type=str2bool, default=False, help="extract features"
    )
    return parser
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(
        ...
        use_random_identity_sampler=False,
        random_identity_num=None,
    ):
        super().__init__()
        ...
        self.use_random_identity_sampler = use_random_identity_sampler
        self.random_identity_num = random_identity_num
    ...
    def _train_dataloader(self):
        ...
        if is_iterable_dataset or self.use_worker_init_fn:
            ...
            init_fn = None
        if self.use_random_identity_sampler:
            return DataLoader(
                self.datasets["train"],
                ...
                batch_sampler=BatchSampler(
                    RandomIdentitySampler(
                        ...
                        data_source=self.datasets["train"],
                        num_instances=self.random_identity_num,
                    ),
                    batch_size=self.batch_size,
                    drop_last=True,
                ),
...
class SetupCallback(Callback):
    ...
    def on_exception(self, trainer, pl_module, exception: KeyboardInterrupt):
...
class ImageLogger(Callback):
    def __init__(
        ...
        self.logger_log_images = {
            pl.loggers.tensorboard: self._tensorboard,
        }
...
class CUDACallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module, **kwargs):
        torch.cuda.synchronize(trainer.strategy.root_device)
        max_memory = (
            torch.cuda.max_memory_allocated(trainer.strategy.root_device) / 2**20
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        default_logger_cfgs = {
            ...
            "wandb": {
                ...
                "params": {
                    "name": nowname,
                    "log_model": "all",
                    "project": opt.name if opt.name else None,
                },
            },
        }
        ...
        if opt.train:
            try:
                trainer.fit(
                    model, data, ckpt_path=opt.ckpt_path if opt.resume else None
                )
            except Exception:
                melk()
                raise
