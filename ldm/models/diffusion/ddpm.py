...
class DDPM(pl.LightningModule):
    ...
    @torch.no_grad()
    def sample(self, batch_size=8, return_intermediates=False):
...
class LatentDiffusion(DDPM):
    ...
    @rank_zero_only
    def on_train_batch_start(self, batch, batch_idx, **kwargs) -> None:
    ...
    #     if extract_feature:
    #         gallery_result = {"gallery_f": features.numpy()}
    #         scipy.io.savemat("gf.mat", gallery_result)
    #
    # def on_test_batch_start(
    #     self, batch: Any, batch_idx: int, dataloader_idx: int
    # ) -> None:
    #     # Toggle this variable to enable/disable extract feature
    #     global features
    #     global extract_feature
    #     extract_feature = False
    #     if extract_feature:
    #         dlen = self.test_dataloader().__len__()
    #         if batch_idx == 0:
    #             features = torch.zeros([dlen * 64, 3, 32, 32])
    #
    #         x = super().get_input(batch, self.first_stage_key)
    #         x = x.to(self.device)
    #         encoder_posterior = self.encode_first_stage(x)
    #         z = self.get_first_stage_encoding(encoder_posterior).detach()
    #         end = min((batch_idx + 1) * 64, dlen * 64, batch_idx * 64 + z.shape[0])
    #         features[batch_idx * 64 : end, :] = z
    #
    ...
    def _get_denoise_row_from_list(
        ...
        inter = [
            self.decode_first_stage(
                samples["x_inter"][i].to(self.device),
                force_not_quantize=force_no_decoder_quantization,
            )
            for i in range(0, len(samples["x_inter"]))
        ]
        for t in inter:
            denoise_row.append(t)
        # denoise_row.append(self.decode_first_stage(samples["x_inter"][-1].to(self.device),
        #                                                     force_not_quantize=force_no_decoder_quantization))
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, "get_text_features") and callable(
                self.cond_stage_model.get_text_features
            ):
                c = self.cond_stage_model.get_text_features(c)[0].unsqueeze(1)
    ...
    def get_input(
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                xc1 = batch[cond_key + "_1"]
                xc2 = batch[cond_key + "_2"]
            else:
                raise NotImplementedError
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc1, dict) or isinstance(xc1, list):
                    ...
                    raise NotImplementedError
                    # c = self.get_learned_conditioning(xc1.to(self.device))
            else:
                c = [i + j for i, j in zip(xc1, xc2)]
            if bs is not None:
                c = c[:bs]
    ...
    def decode_first_stage(
        self, z, predict_cids=False, force_not_quantize=False, extract_feature=False
    ):
        if predict_cids:
            if z.dim() == 4:
        ...
        **kwargs,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                # use only the first caption for better visual effect
                xc = log_txt_as_img((x.shape[2], x.shape[3]), batch["caption_1"])
    ...
    def configure_optimizers(self):
        ...
        opt = schedulefree.AdamWScheduleFree(params, lr=lr)
        # opt = torch.optim.AdamW(params, lr=lr)
        if self.use_scheduler:
